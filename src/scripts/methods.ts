...
export const fetchBooks = async (): Promise<Book[]> => {
  const res = await axios.get('//localhost:7223/api')
...
export const fetchCategories = async () => {
  const database = await axios.get('//localhost:7223/api')
  const cat = database.data.map((book: Book) => book.category)
  const uniquecat = [...new Set(cat)]
  const fam: number[] = []
  for (let i = 0; i < uniquecat.length; i++) {
    const newfam = database.data.find(
      (book: Book) => book.category === uniquecat[i]
    ).family
